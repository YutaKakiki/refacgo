...
func TestRefactoringWithGenAI(t *testing.T) {
	t.Parallel()
	ctrl := gomock.NewController(t)
	mockGenAI := domain.NewMockGenAI(ctrl)
	srcArg := []byte("This is sample code.")
	respString := []string{"This is refactored code!!!", "This is response from Mock!!!"}
	type arg struct {
		src      []byte
		filename string
	}
			...
			mockFunc: func() {
				mockGenAI.EXPECT().QueryResuluts(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).
					DoAndReturn(func(ctx context.Context, src []byte, prompt string, ch chan<- string) error {
						if diff := cmp.Diff(expectedPrompt, prompt); diff != "" {
							t.Errorf("prompt received from RefactoringWithGenAI mismatch (-want +got):\n%s", diff)
						}
						if diff := cmp.Diff(srcArg, src); diff != "" {
							t.Errorf("src received from  mismatch RefactoringWithGenAI mismatch (-want +got):\n%s", diff)
						}
						defer close(ch)
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			tt.mockFunc()
			refactoring := NewRefactoringWithGenAI(mockGenAI)
