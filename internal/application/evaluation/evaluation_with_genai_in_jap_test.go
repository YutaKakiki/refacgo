...
func TestEvauationWithGenAiInJap(t *testing.T) {
	t.Parallel()
	ctrl := gomock.NewController(t)
	mockGenAI := application.NewMockGenAI(ctrl)
	srcArg := []byte("This is sample code.")
	respString := []string{"とてもいいコードです！！", "とてもいいテストコードです！！"}
	type args struct {
		src      []byte
		filename string
	}
	tests := []struct {
		...
		{
			name: "GenAIにソースコード・日本語のプロンプトを正常に渡し、非同期的にチャネルに文字列を送信できる",
			mockFunc: func() {
				mockGenAI.EXPECT().StreamQueryResults(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).Do(
					func(ctx context.Context, src []byte, prompt string, ch chan<- string) {
						...
						expectedPrompt, err := loadfile.LoadInternal("./testdata/prompt/with_genai_in_jap_prompt.txt")
						if err != nil {
							t.Error(err)
						}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			evaluation := NewEvaluationWithGenAiInJap(mockGenAI)
			ch := make(chan string)
